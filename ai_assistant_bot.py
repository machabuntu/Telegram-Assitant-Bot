...
class TelegramWhisperBot:
    ...
    async def _generate_quiz_with_llm(self, topic: str, user, num_questions: int = 10) -> Optional[dict]:
        ...
        prompt = (
            ...
            "- В русских вариантах ответа НЕ используй транслитерацию Поливанова "
            "(например, для японских имён предпочитай распространённые формы 'Сузуки', 'Чика', а не 'Судзуки', 'Тика').\n"
            "- Если правильный ответ — это имя и фамилия (или имя + прозвище и т.п.), "
            "включи в answers оба порядка слов: 'Имя Фамилия' и 'Фамилия Имя' (для обоих языков, если применимо).\n"
